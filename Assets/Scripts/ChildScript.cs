...
public class ChildScript : MonoBehaviour
{
    ...
    public int counterlifesaver=0;
    public bool savernear=false;
    ...
    void Update()
    {
        ...
        if (lifesaver && counterlifesaver == 0 && savernear )
        {
            counterlifesaver ++;
    ...
    void OnTriggerEnter (Collider col)
    {
        ...
        if(col.gameObject.tag == "LifeSaver")
        {
            lifesaver = true;
    ...
    void OnTriggerStay(Collider other) {
        if(other.gameObject.tag == "LifeSaver")
        {
            childdistance = Vector3.Distance(this.transform.position,other.transform.position);

            if(childdistance < 3.0f)
            {
                savernear = true;
                //  LifeSaver lifesave = other.gameObject.GetComponent<LifeSaver>();
                //  lifesave.pathFinished = true;
            }
        }
    }
}